...
with st.sidebar:
    ...
    bg_val     = st.number_input("Gas Expansion (Bg)", value=113.695784, format="%.6f")

    # ── Section 2: Surface Tests ─────────────────────────────
    ...
    d_top  = st.number_input("Top Anchor Depth (m)", value=4333.0, format="%.1f")
    d_bot  = st.number_input("Bot Anchor Depth (m)", value=5031.0, format="%.1f")
    sp_m   = st.number_input("Spinner Slope",         value=0.042,  format="%.4f")
    sp_c   = st.number_input("Spinner Intercept",     value=0.1,    format="%.4f")
    vpcf   = st.number_input("VPCF Factor",           value=0.83,   format="%.4f")
    offset = st.number_input("Tool Offset (m)",       value=18.5,   format="%.2f")
    bh_id  = st.number_input("Borehole ID (in)",      value=6.0,    format="%.2f")
...
if run_clicked:
    ...
    try:
        ...
        def process_physics(raw_df, label=''):
            ...
            v_f = ((spinner_f(m.index) - sp_c) / (sp_m * 60.0)
        ...
        with acol1:
            zs  = sorted(zone_pct_P1.items(), key=lambda x: x[1])
            n_s = [z[0] for z in zs]
            v_s = [z[1] for z in zs]
        ...
        with dd_col:
            ...
            if len(valid) >= 2:
                ...
                ax.plot(xf, re.slope*xf + re.intercept, color='#ff4444', ls='--', lw=2.0,
                        label=f'PI = {re.slope:.4f} m³/d/kPa   R² = {re.rvalue**2:.3f}')
                ax.legend(fontsize=10)
            ax.set_xlabel('Mean ΔP (kPa)', fontsize=10)
            ax.set_ylabel('Q Gas Res (m³/d)', fontsize=10)
            fig.tight_layout(pad=1.6)
            st.pyplot(fig, use_container_width=True)
            plt.close(fig)
        ...
        if len(flowing_tests) > 1:
            ...
            with sc1:
                ...
                ax.bar(xi-wb, tq,                      wb, color='#38c4f0', label='Gas SC',       edgecolor='#08101e')
                ax.bar(xi,   [q*GOR*100 for q in tq],  wb, color='#00e676', label='Oil SC ×100',  edgecolor='#08101e')
                ax.bar(xi+wb,[q*WGR*10  for q in tq],  wb, color='#ff8c00', label='Water SC ×10', edgecolor='#08101e')
        ...
        cal_pts['L_ms'] = cal_pts['LSPD'] * 0.00508
